...
import com.example.ekyc.repository.ApplicationStatusRepository;
...
import com.example.ekyc.util.OCR.converter.ConverterterAllApplicantStatus;
...
//            individualApplicationRepository.save(individualApplicationEntity);
    ...
    public Void addApplicationStatusTbl(String applicationType, UUID applicationId, UUID userID) {
        if (applicationType.equals("individual")) {
            ApplicationStatusEntity applicationStatusEntity = new ApplicationStatusEntity();
            applicationStatusEntity = applicationStatusEntity.builder()
            ...
            applicationStatusEntity = applicationStatusRepository.save(applicationStatusEntity);
...
//                applicationDTO.setOtherDocumentId(individualApplicationEntity.getOtherDocumentId());
            ...
            if (optionalOrganizationApplicationEntity.isPresent()) {
                ...
                ApplicationDTO applicationDTO = ApplicationDTO.builder()
                        ...
                        .otherDocumentId(organizationApplicationEntity.getOtherDocumentId())
                        .build();
...
//           List<ApplicationDTO> applicationDTOList = new ArrayList<>();
    ...
    public List<ApplicationStatusDTO> getAllStatus() {
        List<ApplicationStatusDTO> applicationStatusDTOList = new ArrayList<>();

        List<ApplicationStatusEntity> applicationEntities = applicationStatusRepository.findAll();

        applicationStatusDTOList = applicationEntities.stream()
                .map(ConverterterAllApplicantStatus::convertToDto).toList();

        return applicationStatusDTOList;
    }

    public List<ApplicationStatusDTO> getAllStatusById(UUID userId) {
        List<ApplicationStatusEntity> applicationEntities = applicationStatusRepository.findAllByUserID(userId);
...
//                individualApplicationEntity.setApplicationId(applicationDTO.getApplicationId());
    ...
    public String deleteApplicant(String applicationType, UUID applicantId) {
        if (applicationType.equals("individual")) {
            Optional<IndividualApplicationEntity> optionalIndividualApplicationEntity = individualApplicationRepository.findById(applicantId);
            if (optionalIndividualApplicationEntity.isPresent()) {
